...
from code_env.code_tool import CodeTool

load_dotenv()

app = FastAPI(title="Code Tool Environment", version="0.1.0")
...
COMMAND_SUBSCRIPTION_NAME = os.environ.get("COMMAND_SUBSCRIPTION_NAME", "codesubscription")
            ...
            try:
                ...
                if isinstance(payload, dict):
                    ...
                    if msg_id and msg_id not in app.state._processed_ids and isinstance(data, dict):
                        ...
                        code_command = data.get("code_command")
                        if code_command:
                            tool: CodeTool = getattr(app.state, "code_tool", None) or CodeTool()
                            app.state.code_tool = tool
                            logger: logging.Logger = getattr(app.state, "logger", logging.getLogger("code_tool"))
                            logger.info(f"[code_tool][topic] starting code_command='{code_command}'")
                            try:
                                result = await tool.run(code_command)
                                status = result.get("status", "ok")
                            except Exception as e:
                                status = "error"
                                result = {"error": str(e)}
                            reward_payload: Dict[str, Any] = {
                                "type": "code_tool_results",
                ...
                logging.getLogger("code_tool").exception(f"[worker] error: {e}")
                await asyncio.sleep(1.0)

    app.state.worker = asyncio.create_task(_process_loop(), name="code-tool-worker")
...
@app.get("/")
async def root():
    return {"message": "Code Tool Environment API", "status": "running"}
