...
class CommandQueue:
    """Async receiver identical in shape to web-rl-env but filtering for code type (topic/subscription)."""
    def __init__(self, client: ServiceBusClient, topic_name: str, subscription_name: str) -> None:
        ...
        self._logger = logging.getLogger("code_tool")

    async def start(self) -> None:
        if self._task and not self._task.done():
            return
        self._task = asyncio.create_task(self._run(), name="code-command-subscription-receiver")
            ...
            async with receiver:
                async for msg in receiver:
                    ...
                    if isinstance(parsed, dict):
                        keys_of_interest = ("type", "code_command", "request_id")
                    ...
                    if normalized_type != "code":
                        ...
                        if self._logger:
                            self._logger.info("Not code command, ignored")
                    ...
                    if self._logger:
                        self._logger.info("request accepted by code tool")
