...
app = FastAPI(title="Simple Web RL Environment", version="0.2.5")
...
# Service Bus configuration (accept both names)
SERVICE_BUS_CONNECTION_STRING = (
os.environ.get("SERVICE_BUS_CONNECTION_STRING")
)
COMMAND_TOPIC_NAME = os.environ.get("COMMAND_TOPIC_NAME", "commandtopic")
REWARD_TOPIC_NAME = os.environ.get("REWARD_QUEUE_NAME", "rewardtopic")
COMMAND_SUBSCRIPTION_NAME = os.environ.get("COMMAND_SUBSCRIPTION_NAME", "websubscription")
REWARD_SUBSCRIPTION_NAME = os.environ.get("REWARD_SUBSCRIPTION_NAME", "webrewardsubscription")


# Create clients when configured; otherwise keep None and degrade endpoints
...
async def _startup():
    ...
    if aio_servicebus_client is not None:
        # Start background async receiver (push-style)
        app.state.cmd_queue = CommandQueue(aio_servicebus_client, COMMAND_TOPIC_NAME, subscription_name=COMMAND_SUBSCRIPTION_NAME)
        ...
        app.state.cmd_queue = None
        app.state.reward_queue = None
...
async def health_check():
    try:
        if servicebus_client is None:
            return {"status": "degraded", "error": "Service Bus connection string missing"}
...
async def receive_command(command: CommandRequest):
    try:
        ...
        if servicebus_client is None:
            return {"success": False, "error": "Service Bus not configured"}
